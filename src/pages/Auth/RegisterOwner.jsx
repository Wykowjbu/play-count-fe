  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center bg-background-light dark:bg-background-dark px-6 py-12 lg:px-20 overflow-y-auto w-full h-full">
      <div className="w-full max-w-[480px]">
        ...
        <div className="mb-10">
          <h1 className="text-[#0d1b0f] dark:text-white tracking-tight text-4xl font-bold leading-tight mb-3">Partner with Us</h1>
          <p className="text-gray-500 dark:text-gray-400 text-lg">Register your facility to start receiving bookings.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="mb-8">
            <div className="flex h-14 w-full items-center justify-center rounded-2xl bg-gray-100 dark:bg-white/5 p-1.5">
              <label className="flex cursor-pointer h-full grow items-center justify-center overflow-hidden rounded-xl px-2 has-[:checked]:bg-white dark:has-[:checked]:bg-[#1a2e1d] has-[:checked]:shadow-sm has-[:checked]:text-[#0d1b0f] dark:has-[:checked]:text-primary text-gray-500 dark:text-gray-400 text-sm font-bold transition-all">
                ...
                <input name="role" value="Player" className="invisible w-0" type="radio" onChange={handleRoleChange} />
              </label>
              <label className="flex cursor-pointer h-full grow items-center justify-center overflow-hidden rounded-xl px-2 has-[:checked]:bg-white dark:has-[:checked]:bg-[#1a2e1d] has-[:checked]:shadow-sm has-[:checked]:text-[#0d1b0f] dark:has-[:checked]:text-primary text-gray-500 dark:text-gray-400 text-sm font-bold transition-all">
                <span className="truncate flex items-center gap-2">
                  <span className="material-symbols-outlined text-xl">stadium</span> Court Owner
                </span>
                <input name="role" value="CourtOwner" className="invisible w-0" type="radio" defaultChecked onChange={handleRoleChange} />
              </label>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-gray-700 dark:text-gray-300 ml-1">Business Email Address</label>
              <input type="email" name="email" value={formData.email} onChange={handleChange} required className="w-full h-14 rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#111c12] px-5 focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all placeholder:text-gray-400" placeholder="business@company.com" />
            </div>

            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-gray-700 dark:text-gray-300 ml-1">Phone Number</label>
              <input type="tel" name="phoneNumber" value={formData.phoneNumber} onChange={handleChange} required className="w-full h-14 rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#111c12] px-5 focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all placeholder:text-gray-400" placeholder="business phone number" />
            </div>
