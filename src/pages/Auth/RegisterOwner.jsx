...
export default function RegisterOwner() {
  ...
  return (
    <div className="auth-card">
      <div className="mb-7">
        <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-primary">Owner account</p>
        <h1 className="text-3xl font-black text-slate-900">Register your venue</h1>
        <p className="mt-2 text-sm font-bold text-slate-500">Manage courts, bookings, revenue, and match demand from one dashboard.</p>
      </div>
      ...
      <div className="mb-6 flex rounded-2xl bg-slate-100 p-1.5">
        <label className="flex h-11 grow cursor-pointer items-center justify-center gap-2 rounded-xl px-3 text-sm font-black text-slate-500 transition-all has-[:checked]:bg-white has-[:checked]:text-slate-900 has-[:checked]:shadow-sm">
          ...
          Player
          <input name="role" value="Player" className="sr-only" type="radio" onChange={handleRoleChange} />
        </label>
        <label className="flex h-11 grow cursor-pointer items-center justify-center gap-2 rounded-xl px-3 text-sm font-black text-slate-500 transition-all has-[:checked]:bg-white has-[:checked]:text-slate-900 has-[:checked]:shadow-sm">
          <span className="material-symbols-outlined text-xl">stadium</span>
          Court Owner
          <input name="role" value="CourtOwner" className="sr-only" type="radio" defaultChecked onChange={handleRoleChange} />
        </label>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <AuthInput label="Owner Name" icon="badge" name="fullName" value={formData.fullName} onChange={handleChange} placeholder="Nguyen Van A" />
        <AuthInput label="Business Email" icon="mail" type="email" name="email" value={formData.email} onChange={handleChange} placeholder="business@venue.com" />
        <AuthInput label="Business Phone" icon="phone" type="tel" name="phoneNumber" value={formData.phoneNumber} onChange={handleChange} placeholder="+84 XXX XXX XXX" />
